...
//void PrintApiSetImplementation(const char* apiSetName) {
//    // Convert API set name to wide char
//    WCHAR apiSetNameW[MAX_PATH];
//    MultiByteToWideChar(CP_ACP, 0, apiSetName, -1, apiSetNameW, MAX_PATH);
//
//    // Open the API set schema key
//    HKEY hKey;
//    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\Session Manager\\ApiSetSchema", 0, KEY_READ, &hKey) != ERROR_SUCCESS) {
//        printf("Failed to open ApiSetSchema registry key\n");
//        return;
//    }
//
//    // Query the value of the API set
//    WCHAR dllPath[MAX_PATH];
//    DWORD bufferSize = sizeof(dllPath);
//    DWORD valueType = REG_SZ;
//    if (RegQueryValueExW(hKey, apiSetNameW, NULL, &valueType, (LPBYTE)dllPath, &bufferSize) != ERROR_SUCCESS) {
//        printf("Failed to query ApiSetSchema value\n");
//        RegCloseKey(hKey);
//        return;
//    }
//
//    RegCloseKey(hKey);
//
//    // Convert DLL path to multi-byte string
//    char dllPathA[MAX_PATH];
//    WideCharToMultiByte(CP_ACP, 0, dllPath, -1, dllPathA, MAX_PATH, NULL, NULL);
//
//    // Print the DLL implementing the API set
//    printf("DLL implementing %s: %s\n", apiSetName, dllPathA);
//}
